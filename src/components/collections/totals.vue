<template>
    <v-row class="ma-0 mb-4" v-if="loading">
        <div style="width:20%" class="px-4" v-for="(loader, index) in loaders" v-bind:key="index">
    ...
    <v-row class="ma-0 mb-4" v-else>
        <div style="width:20%" class="px-2" v-for="(total, index) in totals" v-bind:key="index">
        ...
        Total = {{(sumTotals*1).toLocaleString('es-MX', { style: 'currency', currency: 'MXN',})}}
    </v-row>
</template>
