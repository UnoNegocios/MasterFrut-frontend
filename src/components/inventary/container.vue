...
<keep-alive>
    <v-container style="max-width:100vw;">
        ...
        <v-data-table :item-class="itemRowBackground"  :headers="headers" :items="items" class="elevation-0 px-6 py-4"><!-- show-expand -->
            <!-- Header -->
            <template v-slot:top>
                <v-toolbar flat >
...
export default {
    ...
    methods:{
        itemRowBackground: function (item) {
            if((item.ideal_inventory*1)>(item.inventory*1)){
                return 'bg-alert'
            }
        },
        sumField(key) {//suma
