    ...
    export default {
        ...
        computed: {
            ...
            salesLists(){
                if(this.renderComponent == true){
                    return this.$store.state.quotation.quotations.filter(quotation=>quotation.status == 'vendido')
                    .filter(sale=>sale.company_id == this.payroll.company_id)
        ...
        methods: {
            ...
            save(){
                ...
                axios.post(process.env.VUE_APP_BACKEND_ROUTE + "api/v1/payroll/create",Object.assign(this.payroll)).then(response=>{
                    this.close()
                }).catch(error => {
                    this.snackbar = {
